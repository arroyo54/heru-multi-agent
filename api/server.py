...
def _run_agent(agent_key: str, text: str) -> str:
    ...
    if agent_key == "designer":
        return a["chat_designer"].run(
            ...
            "Entrega:\n"
            "1. CONCEPTO CREATIVO: idea central y emoción que debe transmitir\n"
            "2. COPY: headline principal (máx 6 palabras) + subtítulo + CTA\n"
            "3. PROMPT IA DETALLADO (en inglés para Gemini): describe la imagen incluyendo:\n"
            "   - Sujeto principal y acción\n"
            "   - Composición exacta: qué zona ocupa cada elemento (ej: 'upper third reserved for headline text overlay', 'bottom bar for logo and CTA')\n"
            "   - Tipografía en imagen: fuente sans-serif bold, tamaño relativo, color, posición exacta del headline\n"
            "   - Logo heru: posición (ej: bottom-right corner), tamaño relativo, versión (blanco o azul)\n"
            "   - Iluminación, paleta de colores (#1790EC azul heru, #0C3961 azul oscuro), mood\n"
            "   - Formato y ratio (ej: 1:1 para Instagram, 9:16 para Story)\n"
            "4. JERARQUÍA VISUAL: orden en que el ojo debe recorrer la pieza (1º→2º→3º)\n"
            "Máximo 600 palabras."
        )
